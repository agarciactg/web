...
class AcademicGroupCreateAPIView(APIWithCustomerPermissionsMixin, generics.ListAPIView):
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: serializers.CandidateSerializer(many=False),
    ...
    def post(self, request, *args, **kwargs):
        serializer = serializers.CandidateSerializer(
            data=request.data, context={"is_create": True}
        )
        serializer.is_valid(raise_exception=True)
        academic_groups = serializer.save()
        detail = serializers.CandidateSerializer(academic_groups, many=False)
